...
const ServerChannel = ({channel,server,role}:ChannelProps) => {
    const params=useParams()
    const router=useRouter()

    const Icon=iconMap[channel.type]
    return ( 
        <button onClick={()=>{}} className={cn("group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition mb-3",params?.channeId===channel.id && "bg-zinc-700/20 dark:bg-zinc-700")}>
            ...
            {channel.name==="general" && (
                <Lock  className="ml-auto h-4 w-4 text-rose-500 hover:text-rose-600 dark:text-rose-400 dark:hover:text-rose-300 transition"/>
            )}
        </button>
     );
}
